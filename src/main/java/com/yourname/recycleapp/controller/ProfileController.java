...
public class ProfileController {
    ...
    // 사용자 프로필 저장 (전체 저장) - 기존과 동일, 이미 잘 구현됨
    @PutMapping("/{uid}")
    ...
    // 사용자 계정 삭제 (회원 탈퇴) - 기존과 동일, 이미 잘 구현됨
    @DeleteMapping("/{uid}")
    ...
    // 프로필 이미지 업로드 및 URL 저장 - 기존과 동일, 이미 잘 구현됨
    ...
    // 프로필 정보 중 일부 필드만 수정 (부분 업데이트) - 기존과 동일, 이미 잘 구현됨
    @PatchMapping("/{uid}")
